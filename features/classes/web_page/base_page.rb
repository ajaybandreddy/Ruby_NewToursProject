...
class BasePage
  ...
  def navigate_to_application
    @driver.get $environment_variable[ENV['env'].to_sym][:url]
  end

  def login_user_to_application (user_type)
    if user_type == 'authorized'
      login_page = LoginPage.new(@driver)
      login_page.screen_elements[:user_name].send_keys $environment_variable[ENV['env'].to_sym][:user_name]
      login_page.screen_elements[:password_field].send_keys $environment_variable[ENV['env'].to_sym][:password]
